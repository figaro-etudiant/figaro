...
var fpauthentificator = function() {
  ...
  if (this.passport.popin === false) {
    // La verticale peut décider de forcer le comportement popin.
    this.responsiveRun = true;
  }
  else {
  ...
  if (typeof premiumServicesHost !== 'undefined' && premiumServicesHost.match(this.urlRegexp)) {
    this.premiumHost = premiumServicesHost.slice(-1) === '/' ? premiumServicesHost.slice(0, -1) : premiumServicesHost
  }
  else {
  ...
  this.insertDomElement({
    ...
    'attributes' : {
      ...
      'type': 'text/css',
      'media': 'all',
      'href': this.premiumHost + '/sites/default/modules/fp/fp_user_services/themes/assets/css/fp_auth_api.min.css'
    }
  });
...
fpauthentificator.prototype.init = function() {
  ...
  if (!myObj.passport) {
    ...
    if (typeof console !== 'undefined' && typeof console.log !== 'undefined') {
      console.log('Erreur fpAuth : Probleme de definition du passport');
    }
    return false;
  }
      ...
      if (!myObj.knownElements['forms'][i].className.match(myObj.knownElementsClassName)) {
        ...
        switch (formType) {
          case 'form':
            ...
            var actionpremium = !!myObj.knownElements['forms'][i].getAttribute("data-actionpremium");
...
fpauthentificator.prototype.loadUnloggedCommentFormHtml = function(container, callBack) {
  ...
  button.setAttribute('data-action-type', 'comment');
...
fpauthentificator.prototype.loadCommentFormHtml = function(container, callBack) {
  ...
  // Ajout de proprietes de tracking gtm.
  var trackingGTM = '{"customCategorieBU": "navigation", "customActionBU": "enrichment", "customLabelBUcustomCategorieSPE": "commentaires",'
    + '"customIDSPE": "' + container.getAttribute("data-nodenid")
    + '", "customSiteNameSPE": "' + document.domain
    + '", "customActionSPE": "enrichment", "event" : "customEventBUSPE"}';
  button.setAttribute('data-gtm', trackingGTM);
  ...
  var scriptPath = generatedPath['main'] + '&js=true&action-type=comment' + generatedPath['goto'];
  var scriptDomObj = {
    'type' : 'script',
    'attributes' : {
...
fpauthentificator.prototype.commentStatusMessages = function(messageCode) {
  ...
  var commons = {
    ...
    'charte':{
      'type' : 'a',
      'attributes' : {
        'href' : document.location.protocol + "//www.lefigaro.fr/charte_moderation/charte_moderation.html",
        'target' : '_blank'
      },
      'children' : [{
        'type' : 'text',
      ...
      switch (dataType) {
        ...
        case 'action':
          ...
          myObj.knownElements['links'][i].onclick = function() {
            // Determine si Actionpremium ou non
            var actionpremium = !!this.getAttribute("data-actionpremium");
...
fpauthentificator.prototype.buildParcoursPath = function(elem, typeForm, callbackName, actionpremium) {
  ...
  var formAppId, publicValue, updateValue, version, callback, actionType;
  var generatedHref = [];

  // Set l'appid depuis le lien ou les valeurs du passport.
  formAppId = myObj.passport.appIdLogin || ((elem.getAttribute("data-appId")) ? elem.getAttribute("data-appId") : myObj.passport.appId);
  ...
  if (typeForm === 'commentForm') {
    ...
    actionType = (elem.getAttribute("data-action-type")) ? elem.getAttribute("data-action-type") : 'login';
  }
  ...
  if (callback && callback.match(myObj.urlRegexp)) {
    callback = encodeURIComponent(callback);
    generatedHref['baseUrl'] = '&goto_url=' + callback;
  }
  else if (myObj.responsiveRun) {
  ...
  // Mise en string.
  generatedHref['main'] =
    myObj.premiumHost.replace('http:/', 'https:/') + '/fpservice/fp_auth/login?' +
      'app_id=' + formAppId + '&' +
      'public=' + publicValue + '&' +
      'force_update=' + updateValue + '&' +
      'level_name=' + version + '&' +
...
fpauthentificator.prototype.setPassport = function(clientPassport) {
  var myObj = this;
  myObj.passport = {
    'popin': clientPassport.popin ? clientPassport.popin : false,
    'appIdLogin' : clientPassport.appIdLogin || null,
    'appId' : clientPassport.appId,
    'customFormLogo' : (clientPassport.customFormLogo === 'default') ? document.location.protocol + '//plus.lefigaro.fr/sites/default/themes/figaropremium/images/fplinks.png' : clientPassport.customFormLogo,
...
fpauthentificator.prototype.prepareLink = function(linkType, container, key) {
  ...
  link.setAttribute('data-action-type', actionType);
...
var recommanderUpdateLink = function(linkKey, typeLink, count) {
  ...
  if (countSpan === null || typeof countSpan === 'undefined') {
...
fpauthentificator.prototype.pingFidji = function () {
  ...
  if (myObj.premiumHost.match(myObj.defaultPremiumHost)) {
    var remoteId = myObj.getMetaContent('fp:remote_id');
    var appId = myObj.getMetaContent('fp:app_id');
    var url = myObj.getMetaContent('og:url');
    // ON s'assure qu'on a bien les 3 métas nécessaire avant de pinguer.
    if (remoteId && appId && url) {
      var pingUrl = document.location.protocol + '//api.fidji.lefigaro.fr/ping/?url=' + url;
      var scriptDomObj = {
        'type' : 'script',
